...
export const uploadToS3Bucket = async (fileToUpload: File): Promise<string> => {
    ...
    const { data } = await axios.post(`${DEV_URL}/api/s3/objects`, formData, {
...
export const uploadToDynamoDB = async (key: string, formData: Syllabus) => {
    ...
    await axios.post(`${DEV_URL}/api/dynamo/objects`, params, {
...
export const getUnsignedURL = async (key: string) => {
    const { data } = await axios.get(
        `${DEV_URL}/api/s3/objects/unsignedURL/${key}`,
    );
    return data;
};

export const getDynamoDBItems = async () => {
    const { data } = await axios.get(`${DEV_URL}/api/dynamo/objects`);
    return data;
};
