...
export const uploadToDynamoDB = async (key: string, formData: Syllabus) => {
    ...
    await axios.post(`${API_BASE_URL}/api/dynamo/objects`, params, {
...
export const getUnsignedURL = async (key: string) => {
    const { data } = await axios.get(
        `${API_BASE_URL}/api/s3/objects/unsignedURL/${key}`,
    );
    return data;
};

export const getDynamoDBItems = async () => {
    const { data } = await axios.get(`${API_BASE_URL}/api/dynamo/objects`);
    return data;
};
